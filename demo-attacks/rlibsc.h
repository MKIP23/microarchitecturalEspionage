...
// Size bigger than L1D (e.g. 32 KB)
#define EVICTION_BUF_SIZE (32 * 1024)
static uint8_t eviction_buf[EVICTION_BUF_SIZE];

// Software "flush": evict by touching lots of other lines
static inline void flush(void *addr) {
    (void)addr; // unused, we flush the whole cache anyway
    for (int i = 0; i < EVICTION_BUF_SIZE; i += 64) {
        asm volatile("ld x0, 0(%0)" :: "r"(eviction_buf + i) : "memory");
    }
}
